...
function CustomCheckbox({ checked, onChange, label, badge }: {
  checked: boolean;
  onChange: () => void;
  label: string;
  ...
  return (
    <div style={filterItem} onClick={onChange}>
      <div style={{
        width: "14px",
        height: "14px",
        flexShrink: 0,
        ...
        {checked && (
          <svg width="8" height="8" viewBox="0 0 8 8">
            <polyline points="1,4 3,6 7,2" stroke={C.borderGrey} strokeWidth="1.5" fill="none" strokeLinecap="round" strokeLinejoin="round" />
          </svg>
        )}
      </div>
      {badge ? (
      ...
      <div style={{ padding: "16px", overflowY: "auto", flex: 1 }}>
        ...
        <div style={{ marginBottom: "20px" }}>
          ...
          <select
            ...
            onChange={(e) => update({ sortBy: e.target.value as SidebarFilters["sortBy"] })}
          >
            {SORT_OPTIONS.map((o) => (
          ...
          <div style={{ marginTop: "6px" }}>
            <select
              ...
              onChange={(e) => update({ timeRange: e.target.value as SidebarFilters["timeRange"] })}
            >
        ...
        <div style={{ marginBottom: "20px" }}>
          ...
          {tags.map((tag) => (
            <div
              ...
              onClick={() => update({ selectedTag: filters.selectedTag === tag ? null : tag })}
            >
              <span style={{ flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                {tag}
              </span>
              <span style={{ fontSize: "11px", color: C.metaGrey, flexShrink: 0 }}>
                {counts[tag] ?? 0}
              </span>
            </div>
          ))}
